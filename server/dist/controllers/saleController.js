...
const createSale = async (req, res) => {
    ...
    try {
        ...
        // Create sale
        const saleResult = await client.query('INSERT INTO sales (total_amount, user_id, seller_id, payment_method) VALUES ($1, $2, $3, $4) RETURNING id', [total_amount, user_id, seller_id, payment_method]);
        ...
        for (const item of items) {
            ...
            if (productId) {
                ...
                // Create sale item
                await client.query('INSERT INTO sale_items (sale_id, product_id, quantity, price_at_sale) VALUES ($1, $2, $3, $4)', [saleId, productId, quantity, price]);
                // Update stock
                ...
                await client.query('INSERT INTO sale_items (sale_id, service_name, quantity, price_at_sale) VALUES ($1, $2, $3, $4)', [saleId, service_name, quantity, price]);
                saleItemsData.push({ name: service_name, quantity, price });
            }
        }
        await client.query('COMMIT');
        // Prepare and send Telegram notification
        const sellerResult = await database_1.db.query('SELECT name FROM sellers WHERE id = $1', [seller_id]);
        ...
        (0, telegramService_1.sendTelegramNotification)(telegramMsg).catch(err => console.error('Telegram notification error:', err));
...
const getSaleDetails = async (req, res) => {
    const { id } = req.params;
    try {
        const saleResult = await database_1.db.query(`
            SELECT s.*, u.username, sl.name as seller_name
            FROM sales s 
            LEFT JOIN users u ON s.user_id = u.id 
            LEFT JOIN sellers sl ON s.seller_id = sl.id
        ...
        const itemsResult = await database_1.db.query(`
